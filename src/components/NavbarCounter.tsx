...
export function NavbarCounter() {
  ...
  const { data: session } = useSession(); // 2. Obtén el estado de la sesión
  ...
  // 3. Lógica: El contador solo es válido si existe una sesión activa
  const showCount = mounted && session && cartCount > 0;
